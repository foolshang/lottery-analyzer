// ============================================================
// Lottery Analyzer — Version 22
// ============================================================
// CHANGELOG v22:
//   + เพิ่ม: กรอง input รับเฉพาะตัวเลข 0-9 (และ space + newline ใน textarea)
//   + เพิ่ม: ปุ่มดู log การวิเคราะห์ (มุมขวาบนของหน้าผลวิเคราะห์)
//   + เพิ่ม: Popup แสดง log + ปุ่มดาวน์โหลด .txt
// ============================================================
...
export default function App() {
  ...
  const lockHintAt = (idx) => {
    ...
    setTimeout(() => {
      setHints((h) => h.length === 0 ? [{ front:"", back:"" }] : h);
    }, 0);
  ...
  // สร้าง log การวิเคราะห์ — รายละเอียดเต็ม
  const generateLog = () => {
    ...
    // ผลทาย 3 โหมด
    lines.push("──────────────────────────────");
    lines.push("ผลการทาย — 3 โหมด");
    lines.push("──────────────────────────────");
    ["normal", "no_hints", "hints_only"].forEach((mode) => {
      const label = mode === "normal" ? "ปกติ" : mode === "no_hints" ? "ไม่ใช้ hints" : "ใช้ hints อย่างเดียว";
      const pred = lastPreds[mode];
      if (!pred) {
        lines.push(`  ${label}: (ไม่ได้คำนวณ)`);
        return;
      }
      lines.push(`  ${label}: ${pred.slice(0,3).join(" ")} | ${pred.slice(3,6).join(" ")}`);
    });
    lines.push("");

    // ความถี่เลขแต่ละตำแหน่ง (จาก normal)
    if (results.normal) {
      lines.push("──────────────────────────────");
      ...
      for (let pos = 0; pos < 6; pos++) {
        ...
        const pf = iF ? results.normal.posF : results.normal.posB;
    ...
    ["normal", "no_hints", "hints_only"].forEach((mode) => {
      const label = mode === "normal" ? "ปกติ" : mode === "no_hints" ? "ไม่ใช้ hints" : "ใช้ hints อย่างเดียว";
      lines.push(`  [${label}]`);
      const w = weightsAll[mode];
      for (let pos = 0; pos < 6; pos++) {
        const nonZero = Object.entries(w[pos])
          .filter(([d, val]) => val !== 0)
    ...
    if (history.length > 0) {
      ...
      ["normal", "no_hints", "hints_only"].forEach((mode) => {
        ...
        if (arr.length === 0) {
          lines.push(`  ${label}: (ไม่มีข้อมูล)`);
          return;
        }
        const avg = (arr.reduce((s, h) => s + h.modes[mode].hits, 0) / arr.length).toFixed(2);
        const max = Math.max(...arr.map(h => h.modes[mode].hits));
        lines.push(`  ${label}: เฉลี่ย ${avg} | สูงสุด ${max} | จำนวน ${arr.length} รอบ`);
      });
      lines.push("");
    }
    ...
    <div style={{ minHeight:"100vh", background:P.bg, color:P.text, fontFamily:"'Segoe UI',sans-serif", padding:"16px 12px" }}>
        ...
        <div onClick={()=>setShowLog(false)}
          ...
          <div onClick={(e)=>e.stopPropagation()}
            ...
            <div style={{ display:"flex", alignItems:"center", marginBottom:14 }}>
              <div style={{ fontWeight:800, fontSize:15, color:P.blue }}>📋 Log การวิเคราะห์</div>
              <button onClick={downloadLog} style={{ marginLeft:"auto", marginRight:8, background:P.green+"22", color:P.green, border:`1px solid ${P.green}55`, borderRadius:7, padding:"5px 12px", fontWeight:700, fontSize:11, cursor:"pointer" }}>
                ⬇️ ดาวน์โหลด .txt
              </button>
              <button onClick={()=>setShowLog(false)} style={{ background:"transparent", border:"none", color:P.muted, cursor:"pointer", fontSize:20 }}>✕</button>
            </div>
      ...
      <div style={{ maxWidth:640, margin:"0 auto" }}>
        <div style={{ textAlign:"center", marginBottom:14 }}>
          <div style={{ fontSize:10, letterSpacing:4, color:P.accent, textTransform:"uppercase", marginBottom:4 }}>Adaptive Probability Engine v22</div>
          ...
          {!results ? (
            ...
            <div key={animKey}>
              ...
              <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:8 }}>
                <button onClick={()=>setShowLog(true)} title="ดู log การวิเคราะห์"
                  style={{ background:P.blue+"22", color:P.blue, border:`1px solid ${P.blue}55`, borderRadius:7, padding:"5px 10px", fontWeight:700, fontSize:11, cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}>
                  📋 ดู Log
                </button>
              </div>
