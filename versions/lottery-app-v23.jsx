// ============================================================
// Lottery Analyzer — Version 23
// ============================================================
// CHANGELOG v23:
//   - ลบ: โหมด "ไม่ใช้ hints" (no_hints) และ "ใช้ hints อย่างเดียว" (hints_only)
//   ~ แก้: ย้ายปุ่มดู/ดาวน์โหลด Log จากหน้าผลวิเคราะห์ → หน้าสถิติ
//   ~ แก้: Log แสดงเฉพาะข้อมูลโหมดปกติ
//   ~ แก้: หน้าผลวิเคราะห์, หน้าสถิติ, ประวัติ, heatmap, กราฟ — แสดงโหมดเดียว
// ============================================================
    ...
    <div style={{ minHeight:"100vh", background:P.bg, color:P.text, fontFamily:"'Segoe UI',sans-serif", padding:"16px 12px" }}>
      ...
      <div style={{ maxWidth:640, margin:"0 auto" }}>
        <div style={{ textAlign:"center", marginBottom:14 }}>
          <div style={{ fontSize:10, letterSpacing:4, color:P.accent, textTransform:"uppercase", marginBottom:4 }}>Adaptive Probability Engine v23</div>
        ...
        {tab==="stats" && (
          <div>
            ...
            {/* ปุ่ม Log */}
            {history.length > 0 && (
              <div style={{ background:P.card, border:`1px solid ${P.blue}33`, borderRadius:12, padding:14, marginBottom:14, display:"flex", gap:8, alignItems:"center" }}>
                <span style={{ fontSize:11, color:P.muted, flex:1 }}>📋 Log การวิเคราะห์รอบล่าสุด</span>
                <button onClick={()=>setShowLog(true)} disabled={!results}
                  style={{ background:results?P.blue+"22":"transparent", color:results?P.blue:P.muted, border:`1px solid ${results?P.blue+"55":P.border}`, borderRadius:7, padding:"6px 12px", fontWeight:700, fontSize:11, cursor:results?"pointer":"not-allowed" }}>
                  👁️ ดู
                </button>
                <button onClick={downloadLog} disabled={!results}
                  style={{ background:results?P.green+"22":"transparent", color:results?P.green:P.muted, border:`1px solid ${results?P.green+"55":P.border}`, borderRadius:7, padding:"6px 12px", fontWeight:700, fontSize:11, cursor:results?"pointer":"not-allowed" }}>
                  ⬇️ .txt
                </button>
              </div>
            )}
